...
- 현재 발행은 AI 생성과 자동 품질검사 방식이며, 사람이 모든 브리핑을 발행 전에 승인하지 않습니다.
- 실제 브리핑에는 `AI 요약`, 최종 자동 점검 시각, 주장별 원문과 기사별 오류 신고가 표시됩니다.
- 푸시 해지 시 해당 서버 구독정보를 삭제하고, 만료 구독은 최대 30일, 오류 신고는 최대 90일 뒤 자동 삭제합니다.
- 정식 출시 전 운영 주체·법적 문의 창구, 뉴스 사용권, 사람 편집 승인, 복수 뉴스 공급원과 운영 모니터링을 확정해야 합니다.

공개 화면과 README에서 시제품 기능을 실제 운영 기능처럼 설명하지 않습니다. `/studio`의 인물·수치·승인·정정 기록은 향후 B2B 편집 시스템을 보여 주는 **연결되지 않은 UI 시제품**입니다.
...
| `/terms` | 이용약관 | 제공 중 |
| `/studio` | 편집·브랜드 운영 UI 시제품 | 실제 발행·승인·통계와 연결되지 않음 |
...
- 실제 발행 서버에 연결된 사람의 편집 승인·감사 로그는 아직 없습니다. `/studio`는 UI 시제품입니다.
...
2. 실제 편집자가 확인·수정·보류하고 승인해야만 발행되는 서버 워크플로와 감사 로그
3. 기사 제목·요약·썸네일·원문 링크 사용 범위에 대한 뉴스 공급자·언론사 계약 검토
4. NAVER API HUB 단일 장애와 검색 편향을 줄일 두 번째 합법적 뉴스 공급원
5. 잘못된 요약의 수정·회수·필요 시 재알림을 실제 데이터로 남기는 정정 시스템
6. 생성 실패, 7시 30분 발송 실패, 푸시 만료율, AI 비용과 품질 이상에 대한 경보
