...
> 가장 중요한 답: 일반 사용자는 API 키를 입력하지 않습니다. 운영자가 뉴스·AI·푸시 API를 서버에 한 번 설정해 두면, 사용자는 알림 받을 시간과 요일을 고르고 **이 기기에 알림 등록**만 하면 됩니다. 첫 등록 때 로그인과 브라우저 알림 허용은 필요할 수 있습니다.

## 1. 서비스가 실제로 하는 일

아침결의 하루는 아래 순서로 동작합니다.

1. 오늘 오전 6시 15분에 자동 작업이 시작됩니다.
...
예를 들어 화요일 오전 7시에 받는 브리핑은 월요일 00:00~23:59에 보도된 뉴스를 종합한 결과입니다.

```text
월요일 하루의 뉴스
        ↓
화요일 06:15 수집·묶기·AI 요약·교차 확인
        ↓
...
3. 받을 시각을 선택합니다. 기본값은 오전 7시입니다.
4. 받을 요일을 선택합니다.
5. **이 기기에 알림 등록**을 누릅니다.
6. 로그인 화면이 나오면 로그인합니다.
7. 서비스 화면으로 돌아온 뒤 필요한 경우 **이 기기에 알림 등록**을 한 번 더 누릅니다.
8. 브라우저가 알림 허용 여부를 물으면 **허용**을 누릅니다.
9. `등록 완료` 안내가 보이면 끝입니다.
...
알림을 누르면 다음 주소의 카드 영역이 열립니다.

```text
https://morningnews.coders.kr/#delivery-deck
```

운영체제 알림 자체에는 공간 제한이 있으므로 뉴스 요약 전체가 들어가지 않습니다. 알림은 “브리핑이 준비됐다”는 도착 신호이고, 뉴스별 제목·AI 요약·중요성·출처는 알림을 눌러 열린 카드 묶음에서 확인합니다.
...
5. 로그인 화면이 나오면 완료하고 서비스로 돌아옵니다.
6. 브라우저 알림을 **허용**합니다.
7. 초록색 `이 기기는 실제 아침 브리핑 수신 등록이 완료됐습니다` 문구를 확인합니다.
8. 새로 나타난 **내 기기 실제 푸시 테스트** 버튼을 누릅니다.
9. 휴대폰의 잠금화면 또는 알림센터를 확인합니다.
10. 알림을 눌러 카드 영역이 열리는지 확인합니다.

이 버튼은 단순히 브라우저 안에서 알림 모양만 그리는 기능이 아닙니다. 서버가 실제 Web Push 제공 경로로 현재 로그인 계정의 현재 기기에 발송합니다.
...
- 뉴스 종합 자동 시작: 매일 오전 6시 15분, Asia/Seoul
- 기본 사용자 수신 시각: 오전 7시
- 선택 요일: 알림을 받는 날짜의 요일
...
- PWA 푸시: 사용자 기기에 도착 알림을 자동 전송하고 카드 웹 화면을 엶
...
자동 스케줄은 매일 오전 6시 15분에 실행됩니다. 필요하면 관리자 토큰으로 수동 생성을 요청할 수 있습니다.
...
로그인된 운영자 브라우저의 개발자 콘솔에서 배포 서버를 호출하는 예시:
...
- coders.kr의 쓰기 요청 보호 때문에 배포 서버에서 수동 POST를 실행할 때 플랫폼 로그인이 추가로 필요할 수 있습니다.
...
| `POST` | `/api/stories/{storyId}/feedback` | 오류·편향·불명확·도움됨 피드백 | coders.kr 사용자 |
| `GET` | `/api/push/session` | 현재 플랫폼 로그인 여부 | 공개 확인 |
| `GET` | `/api/push/public-key` | 웹푸시 활성 상태와 VAPID 공개키 | 공개 읽기 |
| `POST` | `/api/push/subscriptions` | 현재 기기 푸시 구독 등록·시간 변경 | coders.kr 사용자 |
| `DELETE` | `/api/push/subscriptions` | 현재 기기 푸시 해지 | coders.kr 사용자 |
| `POST` | `/api/push/test` | 로그인 사용자의 현재 기기에 실제 테스트 발송 | coders.kr 사용자 |
| `POST` | `/api/admin/briefings/generate` | 전날 뉴스 수동 생성 | 플랫폼 인증 + 관리자 토큰 |
...
| `BRIEFING_ADMIN_TOKEN` | 수동 생성 | 관리자 생성 엔드포인트 토큰 | 빈 값 |
...
### 등록 버튼을 누르니 로그인 화면으로 이동합니다

정상일 수 있습니다. 푸시 등록은 서버에 쓰기 작업을 하고 구독 소유자를 구분해야 하므로 coders.kr 인증을 사용합니다. 로그인 후 서비스로 돌아와 등록 버튼을 다시 누르고 알림을 허용하세요.
...
주요 동작은 한 버튼으로 시작됩니다. 다만 첫 사용자는 본인 구독을 보호하기 위한 로그인과 운영체제 알림 허용을 한 번 확인해야 합니다. API 키 입력이나 결제는 없습니다.
...
- Deploy: Docker, nginx, coders.kr native mode
...
- 푸시 구독은 로그인 사용자 ID와 묶어 다른 사용자의 테스트 발송을 막습니다.
