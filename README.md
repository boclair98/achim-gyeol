...
5. `무료 알림 받기`를 누릅니다.
6. 정규 도착 시각이 매일 오전 7시 30분으로 안내되는지 확인합니다.
...
기본 설정에서는 매일 오전 7시 30분에 다음 내용이 담긴 푸시 알림이 옵니다.

- NAVER 뉴스 API가 국내 전 분야를 수집하고, GDELT Cloud가 여러 해외 매체가 함께 보도한 국제 중요 사건을 보강하며, 한국은행·금융위원회·질병관리청 공식 RSS를 1차 자료로 연결
- 국내외 기사 언어와 관계없이 독자가 보는 제목·한 줄 결론·요약·핵심 사실·영향·다음 확인점은 모두 자연스러운 한국어로 작성
...
- 8건·5개 분야는 목표치와 운영 경고로 사용합니다. 목표에 못 미쳐도 기사별 검증을 통과한 뉴스는 오전 7시 30분에 발송하며, 중요하지 않은 기사를 분량 채우기용으로 넣지 않습니다.
...
- 각 카드의 `흥미로웠어요` 또는 `관심 없어요`를 누르면 익명 기기 기준으로 90일간 관심 신호를 저장합니다. 다음 브리핑부터 모두가 알아야 할 공통 핵심 3건은 그대로 두고, 나머지 카드의 순서만 관심 분야에 맞게 조정합니다. 뉴스 자체를 숨기거나 AI를 사용자마다 다시 호출하지 않습니다.
...
발송 시각은 모든 사용자에게 한국 시간 매일 오전 7시 30분으로 고정됩니다. 오전 8시를 넘기면 당일 미발송 알림은 보내지 않고 다음 날로 넘어갑니다. 서버나 예약 작업이 늦게 깨어나 9시에 알림이 도착하는 상황을 막기 위한 안전장치입니다.
...
이 값들은 운영자가 서버에 한 번만 설정합니다. 사용자는 브라우저 알림을 허용하면 매일 브리핑을 받습니다.

## 뉴스의 질은 어떻게 관리하나요?

아침결은 기사 한 건을 그대로 줄여 쓰는 방식이 아닙니다.

1. NAVER API HUB 검색 결과를 최신 100건에서 멈추지 않고 최대 3페이지까지 내려가며 전날 국내 기사 후보를 수집합니다. 수집 시점의 최신 기사가 첫 페이지를 채워도 전날 뉴스가 누락되지 않게 합니다.
2. GDELT Cloud에서는 전날 같은 사건을 최소 3개 매체가 보도한 스토리 묶음을 중요도 순으로 가져옵니다. 국제·경제·정치·안보·기술·환경·보건처럼 국내 독자에게 영향이 큰 분야를 보강하고 단일 해외 기사나 노출이 적은 단신은 후보 단계에서 제외합니다.
3. 한국은행·금융위원회·질병관리청의 공식 RSS를 직접 읽어 언론 보도와 맞물린 정책·금융·보건 자료를 1차 출처로 표시합니다. 공식 발표 하나만으로 카드를 만들지 않고 독립 보도와 함께 확인될 때 품질 점수를 높입니다.
4. 분야별 한 개의 넓은 검색어에 의존하지 않고 금리·물가·부동산, 사건·재난·의료, AI·보안·과학처럼 세분화된 검색어를 사용해 놓치는 사건을 줄입니다.
5. 같은 URL과 비슷한 제목을 정리하고, 표현이 달라도 공통 핵심어와 설명이 이어지는 보도는 같은 사건군으로 묶습니다.
6. 동일 사건을 다룬 여러 언론사의 보도를 하나의 사건군으로 만들고 서로 다른 출처가 두 개 이상 연결된 사건만 AI에 전달합니다.
7. OpenAI Structured Outputs가 사건별로 자체 제목, 한 줄 결론, 2~4문장 요약, 왜 중요한지, 다음 확인점, 2~5개의 핵심 사실과 출처 번호를 작성합니다. 외국어 원문도 모든 독자 노출 필드를 한국어로 번역·정리하며, 서버가 한국어가 빠진 필드를 다시 거절합니다.
8. AI가 75점 이상은 전 분야 공통 핵심, 60~74점은 해당 분야에서 알아둘 중요 뉴스, 60점 미만은 단신으로 구분합니다. 단, AI 점수나 기사 수만으로 발행하지 않습니다.
9. 핵심 사실마다 **서로 독립된 출처 두 곳 이상**이 연결됐는지 서버가 다시 검사합니다. 같은 언론사의 여러 URL은 한 출처로 셉니다.
10. AI 요약이 끝난 뒤 자체 제목·한 줄 결론·원문 URL을 다시 비교해 같은 사건이 두 카드로 남는 것을 차단합니다.
11. 분야별 후보 요약은 비용과 처리량의 균형을 위한 `gpt-5.6-terra`가 담당하고, 검증을 통과한 후보 전체는 품질 우선 `gpt-5.6-sol`이 **하루 한 번** 최종 편집합니다.
12. 최종 편집 단계는 새로운 사실을 작성하지 않고 기존 후보 번호만 선택합니다. 국내외 파급력, 중복 제거, 중요도 순서와 분야 균형을 판단합니다.
13. 상위 편집 모델이 지연·오류·형식 불일치로 실패하면 기존의 결정론적 중요도·분야 균형 순서를 즉시 사용합니다. 적용 여부·모델·처리 시간·대체 사유는 에디션 운영 기록에 저장하므로 편집 모델 장애 때문에 아침 브리핑이 사라지지 않습니다.
14. 최종 결과가 8건·5개 분야보다 적으면 운영 경고를 남기되, 기사별 검증을 통과한 뉴스는 정규 푸시로 발송합니다. 분량을 맞추기 위해 중요하지 않은 기사를 끼워 넣지는 않습니다.
15. 출처가 충돌하거나 근거가 부족하면 `아직 확인되지 않은 것`에 표시하거나 발행 후보에서 제외합니다.
16. 같은 날짜의 생성본이 이미 있으면 뉴스·AI API를 불필요하게 다시 호출하지 않습니다. 검증 카드가 0건이면 안내 브리핑을 발송하고 다음 생성 기회에 다시 수집할 수 있습니다.
17. `REVIEW`·`HELD` 상태의 에디션은 공개 API와 날짜별 주소에서 제외해 검수 전 뉴스가 사용자에게 노출되지 않게 합니다.
...
NAVER API HUB와 GDELT Cloud는 **뉴스 발견 경로**로 사용합니다. 브리핑에는 중간 검색 주소가 아니라 가능한 경우 언론사 원문 주소와 출처명을 보존하고, 국내 공식기관 RSS는 1차 자료로 별도 표시합니다.
...
등록 전에는 이미 만들어진 브리핑을 보낼 주소가 없습니다. 오전 8시 이후 등록한 기기는 당일 정규 알림을 뒤늦게 받지 않고 다음 날부터 받습니다. 등록 직후에는 `[운영자 테스트]` 푸시로 기기 연결부터 확인해 주세요.
...
| `GDELT_CLOUD_API_KEY` | 해외 다매체 중요 스토리 검색 키. 서버에만 저장 | 빈 값 |
| `GDELT_NEWS_ENABLED` | GDELT 해외 뉴스 보강 활성화 | `true` |
| `GDELT_MINIMUM_ARTICLE_COUNT` | GDELT 후보에 필요한 최소 보도 기사 수 | `3` |
| `GDELT_MINIMUM_SIGNIFICANCE` | GDELT 후보 중요도 하한(0~1) | `0.45` |
| `OFFICIAL_NEWS_RSS_ENABLED` | 한국은행·금융위원회·질병관리청 공식 RSS 수집 | `true` |
| `OPENAI_API_KEY` | AI 요약 | 빈 값 |
| `OPENAI_CANDIDATE_MODEL` | 사건별 근거 추출·요약 모델 | `gpt-5.6-terra` |
| `OPENAI_EDITOR_MODEL` | 하루 한 번 최종 선정·순위 편집 모델 | `gpt-5.6-sol` |
| `OPENAI_CONNECT_TIMEOUT_SECONDS` | OpenAI 연결 제한 시간 | `10` |
| `OPENAI_READ_TIMEOUT_SECONDS` | 사건 1건당 AI 응답 제한 시간 | `90` |
| `OPENAI_EDITOR_READ_TIMEOUT_SECONDS` | 최종 편집 1회 응답 제한 시간 | `180` |
| `NEWS_EDITORIAL_PASS_ENABLED` | 상위 모델 최종 편집 단계 활성화 | `false` |
| `NEWS_PIPELINE_ENABLED` | 자동 뉴스 생성 활성화 | `false` |
| `NEWS_GENERATE_ON_STARTUP` | 시작 시 한 번 생성 | `false` |
| `NEWS_GENERATION_CRON` | Spring 6필드 cron | `0 0 1 * * *` |
...
| `PUT` | `/api/stories/{storyId}/interest` | 관심·비관심 선택 및 다음 브리핑 카드 순서 반영 | 익명 기기 ID |
| `GET` | `/api/push/session` | 기기 식별 확인 | 익명 기기 ID |
| `GET` | `/api/push/public-key` | 푸시 활성 상태·공개키 | 공개 |
| `POST` | `/api/push/subscriptions` | 푸시 등록(매일 07:30 고정) | 익명 기기 ID |
| `DELETE` | `/api/push/subscriptions` | 푸시 해지 | 익명 기기 ID |
| `POST` | `/api/push/test` | 현재 기기 테스트 푸시 | 익명 기기 ID |
| `GET` | `/api/reader/preferences` | 관심 분야·분량·동의 설정 | 익명 기기 ID |
...
3. 관리자 상태 API에서 `productionReady=true`인지 확인합니다. `stories<8` 또는 `categories<5`이면 `blockReasons`를 운영 경고로 확인하되 정규 발송은 계속합니다.
4. `activeSubscriptions`가 1개 이상인지 확인합니다.
5. 실제 등록 기기에서 테스트 푸시를 보냅니다.
6. 오전 1시 생성 작업과 오전 6시 45분 발송 준비 작업의 GitHub Actions 결과를 확인합니다.
...
│  │  ├─ config/                 # CORS 등 웹 설정
│  │  ├─ domain/                 # JPA 엔티티
│  │  ├─ integration/            # NAVER·GDELT·공식 RSS·OpenAI 클라이언트
│  │  ├─ repository/             # JPA 저장소
│  │  ├─ service/                # 생성·품질검증·푸시
│  │  └─ web/                    # REST API
...
- GDELT 다매체 해외 중요기사 수집과 국내 공식기관 RSS 보강
- 외국어 원문을 포함한 모든 브리핑 필드의 한국어 출력 검증
- OpenAI 기반 구조화 요약
...
- 매일 오전 7시 30분 고정 발송
- 실제 테스트 푸시
- 오전 1시 생성·오전 7시 30분 발송 자동화와 오전 8시 마감 안전장치
