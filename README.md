...
전날의 중요한 뉴스를 AI가 묶고 검증해, 다음 날 아침 **PWA 알림과 읽기 좋은 카드**로 전달하는 뉴스 브리핑 서비스입니다.

- 서비스: [https://morningnews.coders.kr](https://morningnews.coders.kr)
- 기본 도착 시간: 평일 오전 8시 30분
...
5. `무료 알림 받기`를 누릅니다.
6. 시간과 요일을 확인합니다. 기본값은 평일 오전 8시 30분입니다.
...
기본 설정에서는 평일 오전 8시 30분에 다음 내용이 담긴 푸시 알림이 옵니다.
...
알림을 누르면 홍보용 첫 화면이 아니라 실제 뉴스 카드가 있는 `/briefing` 화면으로 이동합니다.

메일이나 카카오톡으로 보내는 서비스가 아닙니다. 브라우저가 보관한 안전한 푸시 구독 주소를 사용해 잠금화면과 알림센터로 전달합니다.

## 언제 어떤 뉴스를 보내나요?

예를 들어 화요일 오전 8시 30분 브리핑에는 월요일 00:00~23:59에 보도된 뉴스를 담습니다.

```text
전날 00:00~23:59 뉴스 수집
              ↓
오전 8:00 사건 묶기·AI 요약·출처 교차 확인
              ↓
오전 8:30 등록 기기로 PWA 알림 발송
              ↓
알림을 눌러 뉴스 카드 열기
```

사용자가 다른 시간을 선택했다면 선택한 시간 이후에 발송합니다. 이미 시간이 지난 뒤 등록한 경우에는 서버가 다음 발송 확인을 실행할 때 준비된 오늘 브리핑을 보냅니다.
...
이 값들은 운영자가 서버에 한 번만 설정합니다. 사용자는 시간·요일을 고르고 브라우저 알림을 허용하면 됩니다.
...
6. 핵심 사실마다 근거 출처가 연결됐는지 다시 검사합니다.
7. 출처가 충돌하거나 근거가 부족하면 불확실성을 표시하거나 발행 후보에서 제외합니다.
...
| `/briefing` | 푸시를 눌렀을 때 열리는 뉴스 카드 | 실제 연동 |
...
등록 전에는 이미 만들어진 브리핑을 보낼 주소가 없습니다. 등록 직후에는 테스트 푸시로 기기 연결부터 확인해 주세요.
...
| `NEWS_GENERATION_CRON` | Spring 6필드 cron | `0 0 8 * * *` |
...
| `POST` | `/api/push/subscriptions` | 푸시 등록·시간 변경 | 익명 기기 ID |
...
6. 오전 8시와 8시 30분 GitHub Actions 결과를 확인합니다.
