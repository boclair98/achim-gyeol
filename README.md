...
9. 분야별 후보 요약은 비용과 처리량의 균형을 위한 `gpt-5.6-terra`가 담당하고, 검증을 통과한 후보 전체는 품질 우선 `gpt-5.6-sol`이 **하루 한 번** 최종 편집합니다.
10. 최종 편집 단계는 새로운 사실을 작성하지 않고 기존 후보 번호만 선택합니다. 중복 제거, 중요도 순서, 분야 균형을 판단하며 서버가 알 수 없는 번호·중복·최소 8건·5개 분야 미달을 다시 차단합니다.
11. 상위 편집 모델이 지연·오류·형식 불일치로 실패하면 기존의 결정론적 중요도·분야 균형 순서를 즉시 사용합니다. 적용 여부·모델·처리 시간·대체 사유는 에디션 운영 기록에 저장하므로 편집 모델 장애 때문에 아침 브리핑이 사라지지 않습니다.
12. 최종 결과가 8건·5개 분야보다 적으면 운영 경고를 남기되, 기사별 검증을 통과한 뉴스는 정규 푸시로 발송합니다. 분량을 맞추기 위해 중요하지 않은 기사를 끼워 넣지는 않습니다.
13. 출처가 충돌하거나 근거가 부족하면 `아직 확인되지 않은 것`에 표시하거나 발행 후보에서 제외합니다.
14. 같은 날짜의 생성본이 이미 있으면 뉴스·AI API를 불필요하게 다시 호출하지 않습니다. 검증 카드가 0건이면 안내 브리핑을 발송하고 다음 생성 기회에 다시 수집할 수 있습니다.
15. `REVIEW`·`HELD` 상태의 에디션은 공개 API와 날짜별 주소에서 제외해 검수 전 뉴스가 사용자에게 노출되지 않게 합니다.
