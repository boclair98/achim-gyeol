...
운영 자동화는 매일 오전 7시 30분에 서버를 깨워 전날 뉴스를 수집하고 OpenAI로 요약합니다. 오전 8시에는 오늘 브리핑이 준비됐는지 다시 확인하고, 누락됐으면 즉시 생성한 다음 푸시를 발송합니다. 따라서 무료 호스팅의 서버가 절전 상태여도 서버 내부 스케줄러에만 의존하지 않습니다.
