...
export function EditorialStudio() {
  ...
  const approveBriefing = () => {
    ...
    setNotice("편집 승인 완료 · 오전 8시 30분 실제 푸시 발행 대기열에 등록했습니다.");
  };

  const exportConfig = () => {
    const blob = new Blob([JSON.stringify({ brand, statuses, schedule: "08:30", channels: ["web-push", "email", "kakao"] }, null, 2)], { type: "application/json" });
...
function DeskPanel({ statuses, updateStatus, readyCount, canPublish, approveBriefing, publishState }: {
  ...
  return <div className="studio-panel">
    <PanelHeader eyebrow="EDITORIAL DESK" title="오늘의 발행 대기열" description="AI 초안을 근거와 위험도 기준으로 검토합니다." action={<button className="studio-primary" onClick={approveBriefing}><Send size={15} />{publishState === "SCHEDULED" ? "오전 8시 30분 예약됨" : canPublish ? "최종 발행 승인" : `${readyCount}/4 검토 완료`}</button>} />
...
function DeliveryPanel() {
  return <div className="studio-panel">
    ...
    <div className="delivery-overview"><div><Clock3 /><span>다음 발행</span><strong>내일 오전 8:30</strong><small>Asia/Seoul · 평일</small></div><div><Users /><span>예상 수신자</span><strong>12,480명</strong><small>동의 상태 정상</small></div><div><Inbox /><span>콘텐츠</span><strong>카드 6장</strong><small>요약 4 · 표지 1 · 정리 1</small></div></div>
