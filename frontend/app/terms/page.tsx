...
export default function TermsPage() {
  return <main className="enterprise-shell"><SiteHeader context="TERMS OF SERVICE" /><article className="legal-document"><header><span>LEGAL DOCUMENT</span><h1>서비스 이용약관</h1><p>공개 베타 기준 · 2026년 8월 12일</p></header><div className="legal-notice"><strong>공개 베타 안내</strong><p>정식 출시 전 운영 주체, 콘텐츠 사용권, 문의 창구와 적용 법률을 확정하고 법률 검토를 거쳐야 합니다. 현재 유료 결제와 후원 기능은 제공하지 않습니다.</p></div>
    <section><h2>1. 서비스의 목적</h2><p>아침결은 전날의 공개 뉴스 검색 결과와 출처 메타데이터를 AI로 정리하고 자동 품질검사를 거쳐 PWA 웹푸시와 모바일 브리핑 화면으로 전달하는 공개 베타 서비스입니다.</p></section>
    <section><h2>2. AI 사용과 한계</h2><p>현재 브리핑은 AI가 자동 생성하며 모든 판을 사람이 발행 전에 검토하지는 않습니다. 요약은 원문 전체나 전문적 조언을 대체하지 않으므로 중요한 판단 전 연결된 원문과 공식 1차 자료를 확인해야 합니다.</p></section>
    ...
    <section><h2>8. 콘텐츠 오류 신고</h2><p>각 실제 브리핑 뉴스의 ‘이 요약에 오류가 있나요?’ 기능에서 문제가 된 문장과 확인 가능한 근거를 접수할 수 있습니다. 일반 문의와 법적 권리 행사 창구는 정식 출시 전 별도로 고지합니다.</p></section>
