...
export default function TrustPage() {
  return <main className="enterprise-shell">
    ...
    <section className="ai-disclosure"><div><span>AI DISCLOSURE</span><h2>AI와 시스템,<br />운영자의 역할을 구분합니다.</h2></div><ol><li><strong>AI</strong><span>기사 군집화, 중요도 판단 보조, 요약·제목·카드 초안 생성</span></li><li><strong>시스템</strong><span>출처군 독립성, 주장별 인용, 충돌·불확실성, 발행 기준 자동 검사</span></li><li><strong>운영자</strong><span>장애와 신고를 확인하고 정정합니다. 현재 모든 판을 발행 전에 직접 승인하지는 않습니다.</span></li><li><strong>독자</strong><span>원문 확인, 기사별 오류 신고, 수신 설정과 철회</span></li></ol></section>
    <section className="correction-public"><div className="trust-section-head"><span>CORRECTION LOG</span><h2>고친 사실도 뉴스의 일부입니다.</h2><p>실제 공개 베타에서 정정한 내용만 날짜·변경 이유와 함께 공개합니다.</p></div><div><article><time><FileClock /> 공개 베타</time><h3>아직 공개된 정정 이력이 없습니다.</h3><p>오류가 확인되면 기존 문구, 수정 문구, 이유와 시각을 이곳에 기록합니다.</p><footer><span>가상 사례는 정정 이력으로 표시하지 않습니다.</span><b>운영 중</b></footer></article></div></section>
